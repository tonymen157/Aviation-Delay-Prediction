...
# Mock the specific imports that run_quality_report.py makes before importing
with patch.dict('sys.modules', {
    'utils': MagicMock(),
    'utils.pipeline_common': MagicMock(),
    'utils.logging_config': MagicMock(),
}):
    # Configure the mocked modules
    sys.modules['utils.logging_config'].setup_logger = MagicMock()
    sys.modules['utils.pipeline_common'].add_project_root_to_path = MagicMock()

    # Also mock external dependencies
    sys.modules['polars'] = MagicMock()
    sys.modules['pl'] = MagicMock()
...
class TestRunQualityReport(unittest.TestCase):
    ...
    @patch('run_quality_report.pl.read_parquet')
    @patch('run_quality_report.Path')
    @patch('run_quality_report.logger')
    def test_main_successful_flow(self, mock_logger, mock_path_cls, mock_read_parquet):
        ...
        with patch('run_quality_report.load_clean_parquet', return_value=mock_df):
    ...
    def test_load_clean_parquet_file_not_found(self, mock_read_parquet):
        ...
        import pathlib
        with patch('run_quality_report.Path'):
            with self.assertRaises(FileNotFoundError):
                rqr.load_clean_parquet(Path('/mock/not_found'))

    @patch('run_quality_report.generate_report')
